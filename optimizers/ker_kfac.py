...
class KerKFACOptimizer(optim.Optimizer):
    def __init__(self,
        ...
        # TODO (CW): Kernel KFAC optimizer now only support model as input
        super(KerKFACOptimizer, self).__init__(model.parameters(), defaults)
    ...
    def _save_input(self, module, input):
        if torch.is_grad_enabled() and self.steps % self.TCov == 0:
            aa = self.CovAHandler(input[0].data, module, kernel=self._kernel)
            # Initialize buffers
            if self.steps == 0:
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if self.acc_stats and self.steps % self.TCov == 0:
            gg = self.CovGHandler(grad_output[0].data, 
                                  module, 
                                  self.batch_averaged,
                                  kernel=self._kernel)
            # Initialize buffers
